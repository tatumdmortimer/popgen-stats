...
avg_ll_expansion = rep_ll_expansion(num)
...
AIC_stats.append(avg_AIC_expansion)
...
#        AIC = 2 * k - 2 * i
#        AIC_vals.append(AIC)
#    avg_AIC = sum(AIC_vals) / float(len(AIC_vals))
#    print avg_AIC
#boot_k(exp)
...
def rep_ll_growth(num):
    ...
    for i in range(0,num):
        ...
        growth_model = growth_func_ex(growth_popt, ns, pts_l)
        growth_ll_opt = dadi.Inference.ll_multinom(growth_model, data)
        growth_vals.append(growth_ll_opt)
    avg_ll_growth = sum(growth_vals) / float(len(growth_vals))
    return avg_ll_growth

avg_ll_growth = rep_ll_growth(num)
print "Average optimized log-likelihood:",avg_ll_growth
avg_AIC_growth = 2 * k - 2 * avg_ll_growth
print "Average AIC:",avg_AIC_growth
AIC_stats.append(avg_AIC_growth)
...
def rep_ll_bottleneck(num):
    ...
    for i in range(0,num):
        ...
        bottleneck_model = bottleneck_func_ex(bottleneck_popt, ns, pts_l)
        bottleneck_ll_opt = dadi.Inference.ll_multinom(bottleneck_model, data)
        bottleneck_vals.append(bottleneck_ll_opt)
    avg_ll_bottleneck = sum(bottleneck_vals) / float(len(bottleneck_vals))
    return avg_ll_bottleneck

avg_ll_bottleneck = rep_ll_bottleneck(num)
...
AIC_stats.append(avg_AIC_bottleneck)

# Bottlegrowth model

bottlegrowth_func = dadi.Demographics1D.bottlegrowth
# Params are nuB,nuF,T; nuB: Ratio of bottleneck population size to ancient pop size, nuF: Ratio of contemporary to ancient pop size,
# T: Time since bottleneck recovery
...
def rep_ll_bottlegrowth(num):
    ...
    for i in range(0,num):
        ...
        bottlegrowth_model = bottlegrowth_func_ex(bottlegrowth_popt, ns, pts_l)
        bottlegrowth_ll_opt = dadi.Inference.ll_multinom(bottlegrowth_model, data)
        bottlegrowth_vals.append(bottlegrowth_ll_opt)
    avg_ll_bottlegrowth = sum(bottlegrowth_vals) / float(len(bottlegrowth_vals))
    return avg_ll_bottlegrowth

avg_ll_bottlegrowth = rep_ll_bottlegrowth(num)
...
AIC_stats.append(avg_AIC_bottlegrowth)
